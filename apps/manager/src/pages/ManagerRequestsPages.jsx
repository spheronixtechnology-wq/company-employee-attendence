  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Team Device Requests</h1>
          <p className="text-slate-400 text-sm mt-0.5">Review and approve your team members' device registration and access requests.</p>
        </div>
        <button onClick={fetchRequests} className="btn-ghost text-sm"><RefreshCw size={14} /> Refresh</button>
      </div>

      {message && (
        <div className={`p-3 rounded-xl border text-sm ${message.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
          {message.text}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {requests.map(req => (
            <div key={req._id} className="card border border-slate-700/60">
              <div
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-white text-sm">{req.userId?.name}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full border capitalize font-medium ${typeColors[req.requestType] || 'text-slate-400'}`}>
                        {req.requestType}
                      </span>
                      {req.status !== 'pending' && (
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${req.status === 'approved' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                          {req.status}
                        </span>
                      )}
                    </div>
              ...
              {expanded === req._id && (
                <div className="mt-4 pt-4 border-t border-slate-700 space-y-4 animate-in slide-in-from-top-2 duration-200">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="col-span-2">
                      <p className="text-slate-500 text-xs mb-0.5">Reason</p>
                      <p className="text-white bg-slate-800/60 p-3 rounded-xl text-sm">{req.reason || 'No reason provided'}</p>
                    </div>
                    {req.requestedDeviceLabel && (
                      <div>
                        <p className="text-slate-500 text-xs flex items-center gap-1 mb-0.5"><Smartphone size={12} className="text-primary-400" /> Device</p>
                        <p className="text-white font-medium">{req.requestedDeviceLabel}</p>
                      </div>
                    )}
                    {req.ipAddress && (
                      <div>
                        <p className="text-slate-500 text-xs flex items-center gap-1 mb-0.5"><Globe size={12} className="text-blue-400" /> Request IP</p>
                        <p className="text-white font-mono text-xs bg-slate-800/80 px-2 py-0.5 rounded border border-slate-700 inline-block">{req.ipAddress}</p>
                      </div>
                    )}
                    {req.deviceFingerprint && (
                      <div>
                        <p className="text-slate-500 text-xs flex items-center gap-1 mb-0.5"><Shield size={12} className="text-emerald-400" /> Hardware Fingerprint</p>
                        <p className="text-emerald-400 font-mono text-xs bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 inline-block">
                    ...
                    {req.requestedUntil && (
                      <div>
                        <p className="text-slate-500 text-xs flex items-center gap-1 mb-0.5"><Clock size={12} className="text-amber-400" /> Access Until</p>
                        <p className="text-white">{new Date(req.requestedUntil).toLocaleDateString('en-IN')}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {req.status === 'pending' && (
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs text-slate-400 mb-1">Decision Note (optional)</label>
                      ...
                      {req.requestType === 'temporary' && (
                        <div>
                          <label className="block text-xs text-slate-400 mb-1">Approve Until</label>
...
export function ManagerLocationRequestsPage() {
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Team Location Requests</h1>
          <p className="text-slate-400 text-sm mt-0.5">Review requests from team members to work from another office location.</p>
        </div>
        <button onClick={fetchRequests} className="btn-ghost text-sm"><RefreshCw size={14} /> Refresh</button>
      </div>
            ...
            <div key={req._id} className="card border border-slate-700/60">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-white text-sm">{req.userId?.name}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${req.requestType === 'temporary_access' ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' : 'bg-blue-500/10 border-blue-500/30 text-blue-400'}`}>
                        {req.requestType === 'temporary_access' ? 'Temporary' : 'Permanent'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-400">{req.requestedLocationId?.officeName} · {new Date(req.createdAt).toLocaleDateString('en-IN')}</p>
                  </div>
                </div>
                {expanded === req._id ? <ChevronUp size={16} className="text-slate-400" /> : <ChevronDown size={16} className="text-slate-400" />}
              </div>
              ...
              {expanded === req._id && (
                <div className="mt-4 pt-4 border-t border-slate-700 space-y-4">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div><p className="text-slate-500 text-xs">Location</p><p className="text-white">{req.requestedLocationId?.officeName}</p></div>
                    {req.requestedFrom && <div><p className="text-slate-500 text-xs">From</p><p className="text-white">{new Date(req.requestedFrom).toLocaleDateString('en-IN')}</p></div>}
                    {req.requestedUntil && <div><p className="text-slate-500 text-xs">Until</p><p className="text-white">{new Date(req.requestedUntil).toLocaleDateString('en-IN')}</p></div>}
                    <div className="col-span-2"><p className="text-slate-500 text-xs mb-0.5">Reason</p><p className="text-white bg-slate-800/60 p-3 rounded-xl text-sm">{req.reason}</p></div>
                  </div>
