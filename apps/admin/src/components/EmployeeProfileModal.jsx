...
export default function EmployeeProfileModal({ isOpen = true, memberId, onClose }) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get(`/admin/employees/${memberId}/profile?preset=${preset}`);
  ...
  const fetchAttendancePage = async (page) => {
    ...
    try {
      ...
      const res = await api.get(`/admin/employees/${memberId}/attendance?page=${page}&limit=10&from=${from}&to=${to}`);
  ...
  const fetchOtPage = async (page) => {
    ...
    try {
      const { from, to } = profileData.period;
      const res = await api.get(`/admin/employees/${memberId}/overtime?page=${page}&limit=10&from=${from}&to=${to}`);
