...
import {
  ...
  Search, Calendar as CalendarIcon, RefreshCw, Loader2, ArrowRight,
  User, Check, X, Shield, MessageSquare, ChevronRight, UserCheck
} from 'lucide-react';
...
export default function SessionReactivationsPage() {
  ...
  const fetchRecords = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/session-reactivations?date=${date}&search=${encodeURIComponent(search)}`);
  ...
  const handleConfirmDecision = async () => {
    ...
    try {
      const res = await api.post(`/admin/session-reactivations/${selectedRecord._id}/decision`, {
        action: actionType,
      ...
      if (res.data?.success) {
        ...
        await fetchRecords();
        // Immediately navigate to its related category
        setActiveFilter(completedAction === 'approve' ? 'approved' : 'rejected');
      } else {
        setActionMessage({ type: 'error', text: res.data?.message || 'Operation failed' });
      }
    } catch (err) {
      if (err.response?.data?.code === 'ALREADY_ACTIVE' || err.response?.data?.message?.includes('already active')) {
        ...
        setActionMessage({
          type: 'error',
          text: err.response?.data?.message || 'Failed to submit decision. Please try again.',
        });
      }
    } finally {
      setSubmitting(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-violet-100 text-violet-700 flex items-center justify-center border border-violet-200 shadow-xs">
              <ShieldAlert size={22} />
            </div>
            Session Reactivations
          </h1>
          <p className="text-slate-500 text-xs mt-1">
            Review auto-checked out employee explanations and approve or close sessions for today
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="flex items-center justify-between mt-0.5">
            <p className="text-[11px] text-slate-400">Continuous silence / outside</p>
            {activeFilter === 'all' && (
        ...
        >
          ...
          <div className="flex items-center justify-between mt-0.5">
            <p className="text-[11px] text-amber-600">Awaiting manager decision</p>
            {activeFilter === 'pending' && (
        ...
        >
          ...
          <div className="flex items-center justify-between mt-0.5">
            <p className="text-[11px] text-emerald-600">Working session active</p>
            {activeFilter === 'approved' && (
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 p-8 shadow-xs">
          ...
          <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
            {activeFilter === 'pending'
              ? 'No pending session reactivation requests for this date.'
                  ...
                  <div className="flex items-start gap-3.5 min-w-[280px]">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-bold text-slate-900 text-sm">{employee?.name || 'Unknown Employee'}</h3>
                  ...
                  <div className="flex-1 max-w-xl bg-slate-50 rounded-xl p-3.5 border border-slate-200/80">
                    ...
                    {record.outOfBoundsReason ? (
                      ...
                      <div className="text-xs text-amber-700 italic flex items-center gap-1.5">
                        <Clock size={13} />
                        No explanation submitted yet by employee.
                      </div>
                    )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl relative">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-base">
                    {actionType === 'approve' ? 'Reactivate Session' : 'Reject Reactivation'}
                  </h3>
            ...
            <p className="text-xs text-slate-600 mb-4">
              {actionType === 'approve' ? (
                <>
                  Approving will <strong>resume this employee's active session</strong>. The suspended time between auto-checkout and now will be deducted as non-working suspension time, preserving accurate work hours.
                </>
              ) : (
                <>
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                placeholder={
                  actionType === 'approve'
                    ? 'E.g., Approved — client meeting confirmed.'
                    : 'E.g., Rejected — unauthorized absence.'
                }
                rows={2}
