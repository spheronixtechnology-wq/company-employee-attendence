...
export default function WifiSettingsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [locRes, methodRes, ipRes] = await Promise.allSettled([
        api.get('/admin/office-locations'),
        api.get('/admin/attendance-method/active'),
        api.get('/admin/current-ip'),
      ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedOfficeId && selectedOfficeId !== 'new') {
        await api.patch(`/admin/office-locations/${selectedOfficeId}`, payload);
        showNotification('success', '✅ Office location & network settings updated successfully!');
      } else {
        const res = await api.post('/admin/office-locations', payload);
  ...
  const handleSwitchToWifi = async () => {
    ...
    try {
      await api.patch('/admin/attendance-method/switch', {
        method: 'wifi_ip',
